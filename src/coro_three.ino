// modified by Emily, week of 2/27

//Libraries
#include <Wire.h> --help with I2C protocol
//#include <Servo.h> --
...
//Initialize motors
#define motorPin1L A0
#define motorPin1R A1
#define motorPin2L A2
#define motorPin2R A3
#define motorPin3L 10
#define motorPin3R 11
...
void setup() {
  ...
  Serial.println("Orientation Sensor Test: begin setup"); 
  Serial.println("");
  ...
  //sets all motors pins to 0 AKA not moving
  calib();   
  delay(5000);
  ...
  Serial.println("Did they all spin???");
  delay(5000);
...
void kiwiFunct(float fwdVal, float sideVal, float thetaVal){
  ...
  ser1 = (int)(map(avg1, -1, 1, 000, 3000));
...
void convertServoToAnalog(){
  if (ser1<1500){
    an1L = (int)(map((float)ser1, 1300, 1500, 255, 0));
    an1R = 0;
  }
  else {
    an1L = 0;
    an1R = (int)(map((float)ser1, 1500, 1700, 0, 255));
  }

  if (ser2<1500){
    an2L = (int)(map((float)ser2, 1300, 1500, 255, 0));
    an2R = 0;
  }
  else {
    an2L = 0;
    an2R = (int)(map((float)ser2, 1500, 1700, 0, 255));
  }

  if (ser3<1350){
    an3L = (int)(map((float)ser3, 1150, 1350, 255, 0));
    an3R = 0;
  }
  else {
    an3L = 0;
    an3R = (int)(map((float)ser3, 1350, 1550, 0, 255));
  }
}
